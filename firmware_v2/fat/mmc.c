...
#define CS_LOW()     (pinSdcardCs = 0)                // MMC CS = L
#define CS_HIGH()    (pinSdcardCs = 1)                // MMC CS = H
#define INS	         (!pinSdcardDetect)               // Card detected   (yes:true, no:false, default:true)
#define WP	         (!pinSdcardWriteProtect)         // Write protected (yes:true, no:false, default:false)

#define FCLK_SLOW()  spiSckSlow()                     // Set slow clock (100k-400k)
#define FCLK_FAST()  spiSckFast()                     // Set fast clock (depends on the CSD)
...
{
	return (BYTE)spiTransfer(dat);
}

/* Alternative macro to transfer data fast */
#define XMIT_SPI_MULTI(src,cnt)  {UINT c=cnt/2; const BYTE *p=src; do {spiTransfer(*p++);spiTransfer(*p++);} while(--c);}
#define RCVR_SPI_MULTI(dst,cnt)	{UINT c=cnt/2; BYTE *p=dst; do {*p++=spiTransfer(0xFF); *p++=spiTransfer(0xFF); } while(--c);}
